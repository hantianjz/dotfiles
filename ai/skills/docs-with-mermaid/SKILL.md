...
- **Never use `\n` with separator lines** (e.g., `"Title\n─────\nitem1\nitem2"`) in node labels — they render poorly across platforms
- Keep labels to a short single line or a brief comma-separated list
- Put detailed breakdowns in prose or tables outside the diagram, not crammed into nodes
- If a component is a simple leaf (e.g., "LED Hardware"), use a plain node instead of a subgraph
